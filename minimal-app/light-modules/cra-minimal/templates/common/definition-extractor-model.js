var DefinitionExtractor = function() {
  ...
  this.getPageContent = function(pageName) {
    var url =
      ctx.request.scheme +
      "://" +
      ctx.request.serverName +
      ":" +
      ctx.request.serverPort +
      ctx.contextPath;
  ...
  this.getContent_FullTreeFromTopAncestor = function(pageName) {
    ...
    var response = this.httpGet(
      url + "/.rest/delivery/pages/v1/" + rootPageName
    );
  ...
  this.toJson = function(object) {
    ...
    var MapperFeature = Java.type(
      "com.fasterxml.jackson.databind.MapperFeature"
    );
    var JsonAutoDetect = Java.type(
  ...
  this.getAllPossibleCompDefsOnATemplateRECURSE = function(
    templateId,
    components
  ) {
    ...
    if (MapUtils.isNotEmpty(areas)) {
      ...
      areas.values().forEach(function(area) {
        ...
        if (MapUtils.isNotEmpty(availableComponents)) {
          availableComponents.values().forEach(function(availableComponent) {
            var id = availableComponent.getId();

            // If not already loaded
            if (!components.containsKey(id)) {
              def = self.getTemplateDefinition(id);
              components.put(id, def);

              // (Recurse) Handle nested components - ie does the component have an area?
              self.getAllPossibleCompDefsOnATemplateRECURSE(id, components);
            }
          });
        }
      });
